...
for i in range(hidden_layers_n):
  ...
  if w is None:
     print("weights for hidden layer " + str(i + 1) + " were not loaded!")
     sys.exit(1)

  weights_copy['wd'].append(tf.Variable(w.initialized_value()))
...
for i in range(conv_layers_n):
  ...
  if w is None:
     print("weights for convolutional layer " + str(i + 1) + " were not loaded!")
     sys.exit(1)

  weights_copy['wc'].append(tf.Variable(w.initialized_value()))
